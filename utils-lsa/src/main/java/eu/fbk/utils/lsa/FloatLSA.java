...
import eu.fbk.utils.math.FloatVector;
 ...
 * @since 1.0
 */
public class FloatLSA {

    /**
     * Define a static logger variable so that it references the
     * Logger instance named <code>FloatLSA</code>.
     */
    static Logger logger = Logger.getLogger(FloatLSA.class.getName());
    ...
    /**
     * Constructs a <code>FloatLSA</code> object.
     */
    public FloatLSA(File UtFile, File SFile, File rowFile, File colFile, File dfFile, int dim, boolean rescaleIdf)
            throws IOException {
     ...
     * Constructs a <code>FloatLSA</code> object.
     */
    public FloatLSA(String root, int dim, boolean rescaleIdf) throws IOException {
     ...
     * Constructs a <code>FloatLSA</code> object.
     */
    public FloatLSA(String root, int dim, boolean rescaleIdf, boolean normalize) throws IOException {
     ...
     */
    public FloatVector mapTerm(String term) throws TermNotFoundException {
        int i = termIndex.get(term);
        ...
        FloatVector nodes = new FloatVector(Uk[i]);
        return nodes;
    } // end mapTerm
     ...
     */
    public FloatVector mapDocument(BOW bow, boolean b) {
        //logger.info("mapDocument " + b);
        int[] indexes = new int[bow.size()];
        float[] values = new float[bow.size()];
        String term;
        int index;
        int tf;
        float tfIdf;
        ...
        if (current < indexes.length) {
            ...
            float[] shorterValues = new float[current];
            System.arraycopy(indexes, 0, shorterIndexes, 0, current);
            System.arraycopy(values, 0, shorterValues, 0, current);
            return new FloatVector(shorterIndexes, shorterValues);
        }
        return new FloatVector(indexes, values);
     ...
     * Returns a document in the VSM.
     */
    public FloatVector mapDocument(BOW bow) {
     ...
     */
    public FloatVector mapPseudoDocument(FloatVector doc) {
        //logger.info("mapPseudoDocument " + doc);
        // N = Uk.rows();
        //float[] pdoc = new float[Uk[0].length];
        FloatVector nodes = new FloatVector(Uk[0].length);
    ...
    public float compare(String term1, String term2) throws TermNotFoundException {
        FloatVector x1 = mapTerm(term1);
        FloatVector x2 = mapTerm(term2);
        return (float) (x1.dot(x2) / Math.sqrt(x1.dot(x1) * x2.dot(x2)));
    }

    public float compare(BOW bow1, BOW bow2) {
        FloatVector d1 = mapDocument(bow1);
        FloatVector d2 = mapDocument(bow2);
        FloatVector pd1 = mapPseudoDocument(d1);
        FloatVector pd2 = mapPseudoDocument(d2);
        return (float) (pd1.dot(pd2) / Math.sqrt(pd1.dot(pd1) * pd2.dot(pd2)));
    }
        ...
        while (true) {
            ...
            if (query.contains("\t")) {
                ...
                try {
                    ...
                    FloatVector vec1 = mapTerm(query);

                    String term = null;
                    Iterator<String> it = terms();
                    while (it.hasNext()) {
                        term = it.next();
                        FloatVector vec2 = mapTerm(term);
                        float cos = (float) (vec1.dot(vec2) / Math.sqrt(vec1.dot(vec1) * vec2.dot(vec2)));
                        map.put(cos, term);
                    }
                    long end = System.nanoTime();
    ...
    public static void main(String[] args) throws Exception {
        ...
        FloatLSA lsi = new FloatLSA(Ut, Sk, r, c, df, dim, rescaleIdf);
    ...
    private static String getHelp() {
        ...
        // Usage
        sb.append("Usage: java -cp dist/jcore.jar -mx2G eu.fbk.utils.lsa.FloatLSA input threshold size dim idf\n\n");
