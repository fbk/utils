...
class Solver {
     ...
     * Logger instance named <code>Solver</code>.
     */
    ...
    void reconstruct_gradient() {
        ...
        if (2 * nr_free < active_size) {
            logger.info("Warning: using -h 0 may be faster");
        }
    ...
    void Solve(int l, QMatrix Q, double[] p_, byte[] y_,
        ...
        logger.debug("initialize alpha_status");
        {
        ...
        logger.debug("initialize active set (for shrinking)");
        {
        ...
        // initialize gradient
        logger.debug("initialize gradient");
        {
            G = new double[l];
            G_bar = new double[l];
            int i;
        ...
        while (true)
        // mycode
        // there are few cases where it loops forever
        //while(iter < 100)
        {
            if (iter > 5000) {
                logger.warn("forced exit after " + iter + " iterations");
                break;
                //System.exit(0);
            }

            // show progress and do shrinking
            logger.trace(iter + " show progress and do shrinking (" + counter + ")");
            if (--counter == 0) {
            ...
            if (select_working_set(working_set) != 0) {
                ...
                logger.trace("reconstruct the whole gradient");
                reconstruct_gradient();
                // reset active set size and check
                logger.trace("reset active set size and check");
                active_size = l;
            ...
            logger.trace("update alpha[" + i + "] and alpha[" + j + "], handle bounds carefully");
            float[] Q_i = Q.get_Q(i, active_size);
            ...
            logger.trace("update alpha_status and G_bar");
            {
        ...
        // calculate rho
        logger.debug("calculate rho");
        si.rho = calculate_rho();

        // calculate objective value
        logger.debug("calculate objective value");
        {
            double v = 0;
            int i;
            for (i = 0; i < l; i++) {
        ...
        logger.debug("put back the solution");
        {
            for (int i = 0; i < l; i++) {
...
public class svm {
    ...
    public static svm_model svm_train(svm_problem prob, svm_parameter param) {
        ...
        logger.info("svm training (" + param.svm_type + ")...");

        if (param.svm_type == svm_parameter.ONE_CLASS ||
                param.svm_type == svm_parameter.EPSILON_SVR ||
                param.svm_type == svm_parameter.NU_SVR) {
            // regression or one-class-svm
            logger.debug("regression or one-class-svm");
            model.nr_class = 2;
            model.label = null;
            model.nSV = null;
            model.probA = null;
            ...
            logger.debug("grouping training data of the same class...");
            ...
            logger.debug("calculating weighted C...");
            ...
            logger.debug("training " + (nr_class * (nr_class - 1) / 2) + " models...");
            ...
            logger.debug(f.length + " decision functions");
            ...
            logger.debug("building problems...");
            int p = 0;
            for (i = 0; i < nr_class; i++) {
        ...
        logger.debug(model.l + " SVs learned");
        return model;
    }
    ...
    public static void svm_predict_values(svm_model model, svm_node[] x, double[] dec_values) {
        if (model.param.svm_type == svm_parameter.ONE_CLASS ||
            ...
            //
            // CHECK IF IT'S POSSIBLE TO OPTIMIZE
        ...
        if (model.param.svm_type == svm_parameter.ONE_CLASS ||
            ...
            for (i = 0; i < nr_class; i++) {
                for (int j = i + 1; j < nr_class; j++) {
                    ...
                    //my code
                    //logger.warn(i + ", " + j + "; " + model.label[i] + " - " + model.label[j] + " has score " + dec_values[pos]);

                    if (dec_values[pos++] > 0) {
                        ++vote[i];

                        //my code
                        //logger.warn(i + ", " + j + "; [" + model.label[i] + "=" + vote[i] + "], " + model.label[j] + "=" + vote[j]);
            ...
            //my code
            ////System.out.println(i + " label " + model.label[0] + " has vote " + vote[0]);

            int vote_max_idx = 0;
            for (i = 1; i < nr_class; i++) {
                //my code
                ////System.out.println(i + " label " + model.label[i] + " has vote " + vote[i]);
...
// mycode
    ...
    public class Result {
        ...
        Result(int c, double v) {
            this.c = c;
            this.v = v;
        }
    }

    // used by svm_multiclass_predict
    public static double[] svm_predict_score(svm_model model, svm_node[] x) {
        if (model.param.svm_type == svm_parameter.ONE_CLASS ||
                model.param.svm_type == svm_parameter.EPSILON_SVR ||
                model.param.svm_type == svm_parameter.NU_SVR) {
                /*double[] res = new double[1];
            ...
            for (i = 0; i < nr_class; i++) {
                for (int j = i + 1; j < nr_class; j++) {
                    if (dec_values[pos++] > 0) {
                        ...
                        //my code
                        logger.warn(
                                dec_values[pos - 1] + "\tI\t" + i + ", " + j + "; [" + model.label[i] + "=" + vote[i]
                                        + "], " + model.label[j] + "=" + vote[j]);

                    } else {
                        ++vote[j];
                        //my code
                        logger.warn(
                                dec_values[pos - 1] + "\tJ\t" + i + ", " + j + "; [" + model.label[i] + "=" + vote[i]
                                        + "], " + model.label[j] + "=" + vote[j]);
            ...
            res[0] = dec_values[0];
            res[1] = model.label[vote_max_idx];
            //return model.label[vote_max_idx];
            return res;
        }
    } // end svm_predict_score

    // mycode
    // used by svm_multiclass_predict
    // invoked by OVA, binary only
    // returns the margin, label predicted, label not predicted
    public static double[] svm_predict_binary(svm_model model, svm_node[] x) {
        if (model.param.svm_type == svm_parameter.ONE_CLASS ||
                model.param.svm_type == svm_parameter.EPSILON_SVR ||
                model.param.svm_type == svm_parameter.NU_SVR) {
			/*double[] res = new double[1];
    ...
    } // end svm_predict_binary

    public static int[] svm_predict_multiclass(svm_model model, svm_node[] x) {
        if (model.param.svm_type == svm_parameter.ONE_CLASS ||
                model.param.svm_type == svm_parameter.EPSILON_SVR ||
                model.param.svm_type == svm_parameter.NU_SVR) {
            double[] res = new double[1];
...
// end mycode
    ...
    // mycode used by OVA with probability
    public static double[] svm_predict_binary_probability(svm_model model, svm_node[] x, double[] prob_estimates) {
        if ((model.param.svm_type == svm_parameter.C_SVC || model.param.svm_type == svm_parameter.NU_SVC) &&
                model.probA != null && model.probB != null) {
            int i;
            int nr_class = model.nr_class;
            ...
            double[] res = new double[3];
            res[0] = dec_values[0];
            res[1] = model.label[prob_max_idx];
            if (prob_max_idx == 0) {
                res[2] = model.label[1];
            } else {
                res[2] = model.label[0];
            ...
            return svm_predict_binary(model, x);
        }
    }
